...
public class FacturaABM {
	...
	public int agregar(int nroMedidor, String direccion, LocalDate fecha, double totalPagar, String observaciones,
		...
		while (i < facturas.size() && retorno == null) {
			...
			if (a.getNroMedidor() == nroMedidor && a.getFecha().getMonthValue() == fecha.getMonthValue()
	...
	public Factura traerFacturaCompleta(int nroSerie, int mes, int anio) throws Exception {
		...
		while (i < facturas.size() && retorno == null) {
			...
			if (f.getNroMedidor() == nroSerie && f.getFecha().getMonthValue() == mes
		...
		if (retorno == null) {
			throw new Exception(
					"Error en capa de negocio, no se encuentra una factura para esa fecha y cliente indicado");
		}
		return retorno;
	}
	...
	public Factura generarFactura(int nroMedidor, int mes, int anio) {
		...
		if (mes % 2 != 0) {
			...
			LocalDate fechaLecturaAnterior = fechaUltimaLectura.minus(2, ChronoUnit.MONTHS);
			...
			try {
				// Traer lectura del mes anterior actual (las lecturas se toman los meses pares)
				ultimaLectura = LecturaABM.getInstance().traerLectura(m, anioFechaUltima, mesFechaUltima);
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
			try {
				// Traer lectura del mes anterior actual (las lecturas se toman los meses pares)
				lecturaAnterior = LecturaABM.getInstance().traerLectura(m, anioFechaAnterior, mesFechaAnterior);
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}

			Tarifa t = m.getTarifa();

			// Si es un medidor residencial
			if (t instanceof TarifaResidencial) {
				TarifaResidencial tarifaR = (TarifaResidencial) t; // Castear la tarifa a residencial

				// castear las últimas dos lecturas a baja demanda
				LecturaBajaDemanda ultimaLbd = (LecturaBajaDemanda) ultimaLectura;
				LecturaBajaDemanda anteriorLbd = (LecturaBajaDemanda) lecturaAnterior;
				// calcular consumo
				int consumo = ultimaLbd.calcularConsumo(anteriorLbd);
			...
			// Si es industrial
				...
				TarifaIndustrial tarifaI = (TarifaIndustrial) t; // castear la tarifa a industrial

				LecturaAltaDemanda ultimaLad = (LecturaAltaDemanda) ultimaLectura;
				LecturaAltaDemanda anteriorLad = (LecturaAltaDemanda) lecturaAnterior;
				// calcular de consumos
				int consumoPico = ultimaLad.calcularPico(anteriorLad);
				int consumoValle = ultimaLad.calcularValle(anteriorLad);
				int consumoResto = ultimaLad.calcularResto(anteriorLad);

				// agregar a los item de factura
				lstItem.add(new ItemFactura("cargo fijo", tarifaI.getCargoFijo(), 1, "fijo", null));
				lstItem.add(new ItemFactura("cargo por consumo pico", tarifaI.getCargoVariablePico(), consumoPico,
						"KWH", null));
				lstItem.add(new ItemFactura("cargo por consumo valle", tarifaI.getCargoVariableValle(), consumoValle,
						"KWH", null));
				lstItem.add(new ItemFactura("cargo por consumo resto", tarifaI.getCargoVariableResto(), consumoResto,
						"KWH", null));
			}
		...
		// Si es par se duplica la factura del mes anterior con una nueva fecha
			...
			try {
				Factura nuevaFactura = FacturaABM.getInstance().traerFacturaCompleta(nroMedidor,
						fechaFacturaAnterior.getMonthValue(), fechaFacturaAnterior.getYear()); // traer anterior
